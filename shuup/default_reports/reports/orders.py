...
from shuup.default_reports.forms import OrderReportForm, OrderLineReportForm
from shuup.default_reports.mixins import OrderReportMixin, OrderLineReportMixin
...
class OrderLineReport(OrderLineReportMixin, ShuupReportBase):
    ...
    def get_data(self):
        data = []
        order_lines = self.get_objects()[:self.report_limit]
        ...
        if "order_line_type" in self.options.keys() and len(self.options["order_line_type"]) == 1:
            self.schema.pop()  # Remove type column if there's only one type
        return self.get_return_data(data, has_totals=False)
